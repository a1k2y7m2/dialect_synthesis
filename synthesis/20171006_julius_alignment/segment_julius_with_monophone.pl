...
$cur_dir = `dirname $0`; chomp($cur_dir);
$scriptname = `basename $0 .pl`; chomp($scriptname);
#$juliusbin="$cur_dir/bin/julius";
$juliusbin="/home/akiyama/ダウンロード/julius-4.4.2/julius/julius";
## julius executable
#if ($^O =~ /MSWin/){
...
## acoustic model
$hmmdefs="$cur_dir/models/hmmdefs_monof_mix16_gid.binhmm"; # monophone model
...
if ($#ARGV != 3) {
   print "$scriptname scp_basename wav_dir lab_dir out_dir\n";
   exit(0);
}
...
foreach $f (@files) {
    ...
    # open(TRANS, "$labdir/$f.lab") || die "Error: cannot open label file \"$labdir/$f.lab\"\n";
    $txt = `cat $labdir/$f.lab | tr '\n' ' '`; chomp($txt);
    push(@words, &modify_labname($txt));
    # while(<TRANS>) {
	 # chomp;
	 #  s/\n/ /g;
	 #  print "$_\n";
  ...
  print $num . "\n";
    ...
    unlink("$outdir/$f.dfa") if (-r "$outdir/$f.dfa");
    unlink("$outdir/$f.dict") if (-r "$outdir/$f.dict");
    open(DFA, "> $outdir/$f.dfa") || die "Error: cannot open $outdir/$f.dfa for writing\n";
    for ($i = 0; $i <= $num; $i++) {
    ...
    print DFA "$str";
    close(DFA);
    open(DICT, "> $outdir/$f.dict") || die "Error: cannot open $outdir/$f.dict for writing\n";
    for ($i = 0; $i <= $num; $i++) {
	$w = shift(@words);
	$str = "$i [w_$i] $w\n";
	$WLIST{"w_$i"} = "$w";
    ...
    close(DICT);
    if ((! -r "$outdir/$f.dfa") || (! -f "$outdir/$f.dfa")) {
	die "Error: failed to make \"$f.dfa\"\n";
    }
    if ((! -r "$outdir/$f.dict") || (! -f "$outdir/$f.dict")) {
	die "Error: failed to make \"$f.dict\"\n";
    }

    #### execute Julius and store the output to log
    $command = "echo $wavdir/$f.wav | $juliusbin -h $hmmdefs -dfa $outdir/$f.dfa -v $outdir/$f.dict";
    $command .= " -palign";
    if ($hlist ne "") {
	$command .= " -hlist $hlist";
    }
    ...
    open(RESULT, "> $outdir/$f.lab") || die "Error: cannot open result file \"$outdir/$f.lab\"\n";
    open(LOG, "$outdir/$f.log") || die "Error: cannot open $outdir/$f.log file\n";
    $sw = 0;
    while(<LOG>) {
	chomp;
    ...
    close(LOG);
    close(RESULT);
 	 unlink("$outdir/$f.log") if (-r "$outdir/$f.log");

    print "Result saved in \"$outdir/$f.lab\".\n";
}
